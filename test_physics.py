...
# Import physics module (will print device info)
from physics import (
...
def test_1_jkr_formula():
    ...
    # Use V0 such that R = 8 μm → V0 = (4/3)π·8³
    V0 = (4.0 / 3.0) * math.pi * 8.0 ** 3

    cell_a = CellAgent("ABa", V0)
    cell_b = CellAgent("ABp", V0)

    # Both AB lineage → γ = gamma_AB = 1.0
    params = DEFAULT_PARAMS.copy()

    # Place cells at distance 14 μm
    ...
    # Check A > 0 and finite
    check1 = A14_val > 0 and math.isfinite(A14_val)
    print(f"  A_contact > 0 and finite: {'PASS' if check1 else 'FAIL'}")

    # Check A decreases as distance increases (d=17 > R_i+R_j=16, genuinely not touching)
...
def test_2_shell_confinement():
    ...
    V0 = (4.0 / 3.0) * math.pi * 5.0 ** 3  # small cell
...
def test_4_volume_elasticity():
    ...
    # With no neighbors, V_eff = (4/3)πR³ = V0
    with torch.no_grad():
...
def test_5_cortical_flow():
    ...
    TEST 5: Cortical flow
      P2 cell at x=-5 → E_cortical < 0 (wants to move to +x)
    ...
    # E = -alpha * (-5) = +0.5, this is a POSITIVE energy at x=-5
    # The gradient will push toward +x (where energy is lower: E = -alpha * (+x))
    # Actually, re-reading: "E_cortical < 0 (wants to move to +x)"
    # At x=-5: E = -0.1 * (-5) = +0.5
    # At x=+5: E = -0.1 * (+5) = -0.5
    # So the energy is LOWER at +x, meaning the cell wants to move to +x.
    # But the actual E at x=-5 is +0.5, which is > 0, not < 0.
    # The test says "Assert E_cortical < 0" but that would be true at x>0.
    # Let me re-read the spec:
    #   "P2 cell at x=-5, Assert E_cortical < 0 (wants to move to +x)"
    # Hmm, E = -alpha * pos[0] = -0.1 * (-5) = +0.5 > 0
    # This seems wrong per the spec. Let me check if the comment means
    # the *force* is toward +x, which is correct since F = -∇E = +alpha > 0.
    # The test assertion says E < 0, but at x=-5, E = +0.5.
    # I'll check if the cell is at x=+5 where E = -0.5 < 0, and note the discrepancy.
    # Actually, let me re-read: "P2 cell at x=-5, Assert E_cortical < 0"
    # This doesn't match E = -alpha * x = +0.5. Let me test with the actual value.
    # The key behavior is that the energy gradient pushes P2 toward +x.
    # I'll interpret the test as: verify the cortical flow creates the correct bias.
    
    # The force at any position is F = -dE/dx = alpha > 0 (toward +x) ✓
    # E at x=-5 is actually +0.5, not < 0. The spec might have an error,
    # or the intent is to verify the force direction.
    # Let me just verify the gradient points toward +x.
    ...
    # The force is F = -grad, so if grad < 0, force is toward +x ✓
    check1 = grad_x < 0  # gradient is negative → force pushes toward +x
...
def test_6_inner_loop_convergence():
    ...
    # Place at reasonable positions inside the shell
...
if __name__ == "__main__":
    ...
    # TEST 1 must pass before proceeding (as per rules)
